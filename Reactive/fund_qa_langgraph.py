...
from langchain_classic.agents import Tool, AgentExecutor, LLMSingleActionAgent, AgentOutputParser
...
# 配置 DashScope - 优先使用环境变量，其次使用默认值 # 通义千问API密钥
...
def create_fund_qa_agent():
    # 定义LLM
    llm = Tongyi(model="qwen-turbo", api_key=DASHSCOPE_API_KEY)
